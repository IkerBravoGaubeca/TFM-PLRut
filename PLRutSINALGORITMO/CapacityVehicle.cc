...
void CapacityVehicle::finish()
{
    ...
    if (!arrivedSent_ && routeSent_ && !destAddress_.isUnspecified()) {
        EV << "[CapacityVehicle " << carId
           << "] finish() fallback - sending arrived." << endl;
...
void CapacityVehicle::processReply(Packet* pPacket)
{
    ...
    if (hasNewRoute) {
        ...
        EV << "[CapacityVehicle " << carId
           << "] NEW ROUTE received: " << strRoutes << endl;
        ...
        try {
            ...
            EV << "[CapacityVehicle " << carId
               << "] ERROR changing route: " << e.what() << endl;
        }
    }
    else if (wait) {
        EV << "[CapacityVehicle " << carId
           << "] Server says WAIT. Stopping vehicle." << endl;
...
void CapacityVehicle::initTraffic()
{
    ...
    if (destModule == nullptr) {
        ...
        EV << simTime() << " [CapacityVehicle " << carId
           ...
           << ", sending to server port " << destPort_
           << endl;
...
void CapacityVehicle::sendCapacityPacket()
{
    ...
    if (arrived) {
        EV << "[CapacityVehicle " << carId
           << "] Last edges - releasing reservation." << endl;

        arrivedSent_ = true;
    }
    else if (!routeSent_) {
        EV << "[CapacityVehicle " << carId
           << "] SEND route currentEdge=" << currentEdge
        ...
        EV << "[CapacityVehicle " << carId
           << "] UPDATE position currentEdge=" << currentEdge
           << endl;
    }
