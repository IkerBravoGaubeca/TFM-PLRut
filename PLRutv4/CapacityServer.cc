...
static const int WINDOW_AHEAD = 4;
...
void CapacityServer::initialize(int stage)
{
    ...
    if (mapName.find(".net.xml") == std::string::npos)
        mapName += ".net.xml";
    ...
    enableMaxDistance_ = false;
    maxDistance_ = 0;

    if (hasPar("enableMaxDistance"))
        enableMaxDistance_ = par("enableMaxDistance");

    if (hasPar("maxDistance"))
        maxDistance_ = par("maxDistance");
    ...
    const std::string& currentEdge)
{
    ...
    if (pos == fullRoute.end()) {
        int end = std::min((int)fullRoute.size() - 1, WINDOW_AHEAD);

        for (int i = 0; i <= end; i++)
            window.push_back(fullRoute[i]);

        EV << "[CapacityServer] Current edge not found in route. "
           << "Using forward initial window [0.." << end << "] "
           << "size=" << window.size()
           << endl;
    ...
    int start = idx;
    int end   = std::min((int)fullRoute.size() - 1, idx + WINDOW_AHEAD);

    for (int i = start; i <= end; i++)
        window.push_back(fullRoute[i]);

    EV << "[CapacityServer] Forward window from edge=" << currentEdge
       << " [" << start << ".." << end << "]"
       << " size=" << window.size()
       << endl;
                                  ...
                                  const std::string& currentEdge)
{
    ...
    std::vector<std::string> oldWindow =
        (itRes != reservedRoutesByCar_.end()) ? itRes->second : std::vector<std::string>();
        ...
        if (newSet.find(e) == newSet.end()) {
            if (edgeOccupation_.count(e) && edgeOccupation_[e] > 0) {
                ...
                EV << "[CapacityServer] Window release car=" << carId
                   << " edge=" << e
                   << " occ=" << edgeOccupation_[e]
                   << "/" << getCapacityForEdge(e)
                   << endl;
            }
        }
    }
...
void CapacityServer::handleMessage(cMessage *msg)
{
    ...
    if (isWaiting) {
        ...
        if (!windowBlockedForCar(actualCarId, window)) {
            ...
            std::string to = destinationEdge.empty()
                ? originalDestinationByCar_[actualCarId]
                : destinationEdge;
    ...
    // --- Coche ya conocido: actualizar ventana ---
    ...
    if (currentRoute.empty() || currentEdge.empty() || destinationEdge.empty()) {
        EV << "[CapacityServer] Incomplete data from car=" << actualCarId
           << ". Ignoring."
           << endl;
        return;
    }
    ...
    if (!windowBlockedForCar(actualCarId, window)) {
        EV << "[CapacityServer] Initial forward window OK for " << actualCarId << endl;
    ...
    EV << "[CapacityServer] Forward window blocked for new car=" << actualCarId
       << ". Computing alternative from=" << from
       << " to=" << to
       << endl;
    ...
    EV << "[CapacityServer] No alternative for car=" << actualCarId
       << ". Adding to waiting list WITHOUT reserving."
       << endl;
    ...
    // FIX: eliminadas las dos lineas que reservaban aqui
    // fullRouteByCar_[actualCarId] = currentRoute;  <- eliminado
    // reserveWindow(actualCarId, window);            <- eliminado
...
{
    if (waitingCars_.empty())
        return;

    EV << "[CapacityServer] Retrying " << waitingCars_.size()
       << " waiting cars."
       << endl;
    ...
    for (auto& wc : waitingCars_) {
        ...
        fullRouteByCar_.erase(wc.carId);
        ...
        // FIX: sigue sin hueco -> stillWaiting SIN reservar nada
                                         ...
                                         const std::vector<std::string>& window)
{
    for (const auto& edge : window) {
        ...
        if (reservedRoutesByCar_.count(carId)) {
            ...
            if (std::find(own.begin(), own.end(), edge) != own.end())
                occ--;
        }
...
void CapacityServer::loadCapacityJson(const std::string& path)
{
    ...
    if (!file.is_open()) {
        EV << "[CapacityServer] Cannot open: " << path << endl;
        return;
    }
    ...
    EV << "[CapacityServer] Loaded " << edgeCapacity_.size()
       << " edge capacities."
       << endl;
}
...
void CapacityServer::releaseRoute(const std::string& carId)
{
    ...
    if (it == reservedRoutesByCar_.end())
        return;
                                    ...
                                    const std::string& toEdge)
{
    ...
    archivo << "<routes>\n";
    archivo << "    <trip id=\"0\" depart=\"0.00\" from=\"" << fromEdge
            << "\" to=\"" << toEdge << "\"/>\n";
    archivo << "</routes>\n";
    ...
    const std::string& carId)
{
    ...
    if (fromEdge.empty() || toEdge.empty() || fromEdge == toEdge)
        return empty;
    ...
    if (system(("cp \"" + mapName + "\" \"" + netFile + "\"").c_str()) != 0)
        return empty;

    if (!filtered.empty()) {
        EV << "[CapacityServer] Blocking " << filtered.size()
           << " edges for car=" << carId
           << endl;
    ...
    if (edges.empty()) {
        EV << "[CapacityServer] No edges found for car=" << carId << endl;
        return empty;
    }

    std::replace(edges.begin(), edges.end(), ' ', ',');

    EV << "[CapacityServer] Alternative route for car=" << carId
       << ": " << edges
       << endl;
