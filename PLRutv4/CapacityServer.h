...
class CapacityServer : public omnetpp::cSimpleModule
{
    ...
    // Ventana deslizante
    std::vector<std::string> computeWindow(const std::vector<std::string>& fullRoute,
                                           const std::string& currentEdge);
    void updateWindow(const std::string& carId, const std::string& currentEdge);
    void reserveWindow(const std::string& carId, const std::vector<std::string>& window);
    bool windowBlockedForCar(const std::string& carId, const std::vector<std::string>& window);
    ...
    // FIX: buscar módulo OMNeT++ usando el índice correcto
    cModule* findCarModule(const std::string& carId);
...
protected:
    ...
    std::map<std::string, std::vector<std::string>>  fullRouteByCar_;
    std::map<std::string, std::string>               originalDestinationByCar_;
    std::list<WaitingCar>                            waitingCars_;

    // FIX: mapa sumoId (carId) -> índice OMNeT++ real del módulo car[X]
