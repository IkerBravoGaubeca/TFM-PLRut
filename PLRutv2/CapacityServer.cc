...
void CapacityServer::releasePassedEdges(const std::string& carId,
                                        const std::string& currentEdge)
{
    auto it = reservedRoutesByCar_.find(carId);
    if (it == reservedRoutesByCar_.end()) return;

    std::vector<std::string>& route = it->second;

    auto pos = std::find(route.begin(), route.end(), currentEdge);
    if (pos == route.end()) return;

    bool anyReleased = false;
    for (auto e = route.begin(); e != pos; ++e) {
        if (edgeOccupation_.count(*e) && edgeOccupation_[*e] > 0) {
            edgeOccupation_[*e]--;
            EV << "[CapacityServer] release passed edge car=" << carId
    ...
    if (anyReleased) {
        ...
        EV << "[CapacityServer] Updated reserved route for car=" << carId
           << " remaining edges=" << route.size() << endl;
        retryWaitingCars();
    }
}
...
void CapacityServer::handleMessage(cMessage *msg)
{
    ...
    if (isWaiting) {
        ...
        if (!routeBlockedForCar(actualCarId, currentRoute)) {
            ...
            if (!newRoute.empty() && !routeBlockedForCar(actualCarId, newRoute)) {
                ...
                // FIX: sigue bloqueado -> NO reservar nada, el coche sigue en waiting
                // sin ocupar plazas en edgeOccupation_
    ...
    if (reservedRoutesByCar_.count(actualCarId) && !currentEdge.empty()) {
        ...
        for (const auto& id : toRelease) {
            EV << "[CapacityServer] Car " << id
               << " gone. Releasing remaining route." << endl;
            releaseRoute(id);
            originalDestinationByCar_.erase(id);
        }
        if (!toRelease.empty()) retryWaitingCars();

        releasePassedEdges(actualCarId, currentEdge);
        return;
    }
    ...
    if (!newRoute.empty() && !routeBlockedForCar(actualCarId, newRoute)) {
        ...
        // reserveRoute(actualCarId, currentRoute);  <- eliminado
...
void CapacityServer::retryWaitingCars()
{
    ...
    for (auto& wc : waitingCars_) {
        ...
        if (!routeBlockedForCar(wc.carId, wc.originalRoute)) {
            EV << "[CapacityServer] Original route free for car=" << wc.carId << endl;
